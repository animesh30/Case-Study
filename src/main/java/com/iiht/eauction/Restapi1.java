...
public class Restapi1 {
	...
	@CrossOrigin()
	@RequestMapping(value = "/e-auction/api/v1/buyer/update-bid/{Id}/{email}/{bidamount}",method=RequestMethod.POST, headers = "Accept=application/json"  )
	public Bids updatebid(@PathVariable int Id,@PathVariable String email ,@PathVariable int bidamount) {
		Bids bid1  =new Bids();
		
		bid1 = bidsRepository.findByProductidAndEmail(Integer.toString(Id),email);
